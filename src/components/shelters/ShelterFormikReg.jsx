...
export default function ShelterFormikReg() {
  ...
  //FETCH EN MODO POST PARA SHELTER
  const saveShelter = async (values) => {
    console.log("SE INTENTA FETCH");

    setIsLoading(true);
    const newShelter = { ...values };

    try {
      const response = await fetch(`${BASE_URL}shelter/signup`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(
          `Error durante la creación de asociación: ${response.status}, ${data.message}, ${data.error}`
        );
      }

      console.log("Cuenta de protectora creado correctamente ", data);
      setReply({ value: data.message, color: "text-greenL" });
      setIsSuccess(true);
    } catch (error) {
      console.log("Error creando el perfil: ", error);
  ...
  const validationSchemaYup = object({
    email: string().required(
      "El email es obligatorio como contacto principal e inicio de sesion"
    ),
    pswd: string()
      .required("No puedes dejar la contreña vacía")
    ...
    tipoNIF: string()
      .required("Es obligatorio indicar el tipo de NIF")
      .oneOf(["DNI", "NIE", "CIF"]),
    NIF: string().notRequired(),
    province: string().required("Indicar la provincia").oneOf(provincias),
          ...
          {({ values }) => (
            <Form className="bg-pink-softest my-3 rounded-xl border shadow-xl p-2 max-w-md">
              ...
              <div className="flex flex-col justify-between items-center m-1">
                ...
                <div className="w-full">
                  ...
                  <ErrorMessage
                    name="email"
                    component="div"
                    className="bg-red-dark rounded text-white text-sm mt-1 p-1"
                  />
                </div>
              </div>
              {/* CONTRASEÑA */}
              ...
              <div className="flex justify-center m-3">
                ...
                >
                  {isLoading ? "Actualizando..." : "Crear cuenta"}
                </button>
              </div>
              <div>
